...
generated quantities {
  ...
  vector[N] log_lik;
  for (n in 1:N){
    y_rep[n] = neg_binomial_2_log_rng(mu+beta[ww[n]]*w1[n],phi);
  }
  for (n in 1:N){
    log_lik[n] = neg_binomial_2_log_lpmf(y[n]|mu+beta[ww[n]]*w1[n],phi);
  }
  for (i in 1:N_pred){
    y_pred[i] = neg_binomial_2_log_rng(mu+beta[W_pred[i]]*w1_pred[W_pred[i]],phi);
