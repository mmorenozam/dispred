...
def resampler2(x): #resampler for the sum of the sums, from here I can take counts and serves for rain
    if i==0:
        return x.resample('D').mean().rolling(window=1).mean()[0::1]
    else:
        return x.resample('D').mean().rolling(window=i+1).mean()[0::1]
